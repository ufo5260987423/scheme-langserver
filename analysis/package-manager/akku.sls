(library (scheme-langserver analysis package-manager akku)
  ...
  (import 
    ...
    (only (srfi :13 strings) string-suffix? string-prefix? string-contains string-index string-drop string-drop-right))
  
(define (private:hex-digit? c)
  (or (char<=? #\0 c #\9) (char<=? #\a c #\f) (char<=? #\A c #\F)))

(define (private:percent-decode s)
    ...
    (let loop ([i 0] [acc '()])
      (if (>= i n)
        ...
        (let ([c (string-ref s i)])
          (if (and (char=? c #\%) (< (+ i 2) n)
                   (private:hex-digit? (string-ref s (+ i 1)))
                   (private:hex-digit? (string-ref s (+ i 2))))
            (let ([v (or (string->number (substring s (+ i 1) (+ i 3)) 16) 0)])
              (loop (+ i 3) (cons (integer->char v) acc)))
            (loop (+ i 1) (cons c acc))))))))
