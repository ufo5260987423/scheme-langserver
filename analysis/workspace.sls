...
(define (private-init-references workspace-instance target-path . maybe-syntax-diagnoses)
  (let* ([current-file-node (walk-file (workspace-file-node workspace-instance) target-path)]
    ...
    (private:check-undefined-identifiers document)
...
(define (private:check-undefined-identifiers document)
  (let f ([stack (document-index-node-list document)])
    (if (null? stack)
      '()
      (let* ([current (car stack)]
          [rest (cdr stack)]
          [children (index-node-children current)]
          [expression (annotation-stripped (index-node-datum/annotations current))])
        (if (and (not (null? children)) (symbol? (annotation-stripped (index-node-datum/annotations (car children)))))
          (let* ([head (car children)]
              [head-expression (annotation-stripped (index-node-datum/annotations head))])
            (try
              (if (not (or (library-identifier? document head)
                          (private:in-library-header? head)
                          (private:in-binding-position? head)))
                (let ([refs (find-available-references-for document head head-expression)])
                  (if (null? refs)
                    (append-new-diagnoses document (list (index-node-start head) (index-node-end head) 1 (string-append "Undefined identifier: " (symbol->string head-expression)) "identifier" "undefined-identifier")))))
              (except c [else '()]))))
        (f (append (if (and (list? expression) (not (null? expression)) (memq (car expression) '(quote quasiquote syntax quasisyntax)))
                '()
                children)
            rest))))))

(define (private:in-library-header? index-node)
  (let f ([current index-node])
    (let ([parent (index-node-parent current)])
      (if (null? parent)
        #f
        (let ([expr (annotation-stripped (index-node-datum/annotations parent))])
          (if (and (list? expr) (not (null? expr)))
            (case (car expr)
              [(library define-library)
               (let ([siblings (index-node-children parent)])
                 (and (>= (length siblings) 2)
                   (or (eq? current (cadr siblings))
                     (and (memq current (cddr siblings))
                       (let ([child-expr (annotation-stripped (index-node-datum/annotations current))])
                         (and (list? child-expr) (not (null? child-expr))
                           (memq (car child-expr) '(export import only rename prefix except alias))))))))]
              [(export import only rename prefix except alias)
               #t]
              [else (f (index-node-parent parent))])
            (f (index-node-parent parent))))))))
(define (private:in-binding-position? index-node)
  (let ([parent (index-node-parent index-node)])
    (if (null? parent)
      ...
      (let ([grandparent (index-node-parent parent)])
        (if (null? grandparent)
          #f
          (let ([gp-expr (annotation-stripped (index-node-datum/annotations grandparent))])
            (if (and (list? gp-expr) (not (null? gp-expr)))
              (if (memq (car gp-expr) '(define define-syntax))
                (and (>= (length (index-node-children grandparent)) 2)
                  (eq? parent (cadr (index-node-children grandparent))))
                (if (memq (car gp-expr) '(let let* letrec letrec*))
                  (and (not (null? (index-node-children grandparent)))
                    (eq? parent (car (index-node-children grandparent)))
                    (let ([ggp (index-node-parent grandparent)])
                      (and (not (null? ggp))
                        (let ([ggp-expr (annotation-stripped (index-node-datum/annotations ggp))])
                          (and (list? ggp-expr) (not (null? ggp-expr))
                            (memq (car ggp-expr) '(let let* letrec letrec*))
                            (eq? parent (car (index-node-children grandparent))))))))
                  (if (memq (car gp-expr) '(lambda case-lambda))
                    (and (>= (length (index-node-children grandparent)) 2)
                      (eq? parent (cadr (index-node-children grandparent))))
                    #f)))
              #f)))))))
