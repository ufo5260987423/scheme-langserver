(library (scheme-langserver analysis identifier rules library-define)
  ...
  (import 
    ...
    (scheme-langserver analysis identifier references)
...
(define (library-define-process root-file-node document index-node)
  (let* ([ann (index-node-datum/annotations index-node)]
    ...
    (match expression
      [('library _ **1 ) 
        (map (lambda (child-node) (match-define root-file-node document child-node)))
        (index-node-children index-node)]
      [else 
        (map (lambda (child-node) (library-define-process root-file-node document child-node)))
        (index-node-children index-node)])))
