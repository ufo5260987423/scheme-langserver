...
; procedure parameter variable syntax-transformer syntax-parameter syntax-variable
              ...
              (lambda (dummy-index-node)
                (let* ([dummy-ann (index-node-datum/annotations dummy-index-node)]
                  ...
                  (match dummy-expression
                    ...
                    [else '()])))
              dummies))]
        [('define-syntax (? symbol? identifier) dummy ... ) 
          (guard-for document index-node 'define-syntax '(chezscheme) '(rnrs) '(rnrs base) '(scheme))
