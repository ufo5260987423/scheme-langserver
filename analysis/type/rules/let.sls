...
(define (let-process index-node document)
  (let* ([ann (index-node-datum/annotations index-node)]
    ...
    (try
      (match expression
        [('let (? symbol? loop-identifier) (((? symbol? identifier) value ) ... ) _ ... ) 
          (guard-for document index-node 'let '(chezscheme) '(rnrs) '(rnrs base) '(scheme))
          (let ([loop-reference-list (private-process (cadr (index-node-children index-node)) index-node '() document 'let-lopp)])
