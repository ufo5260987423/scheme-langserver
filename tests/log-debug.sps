    ...
    (let ([current-line (car lines)])
      (cond 
        ...
        [(and read? (equal? current-line "send-message"))
          (loop (cdr lines) (process result batch) '() #f)]
        [(and (not read?) (equal? current-line "read-message")) 
          (loop (cddr lines) result '() #t)]
        [else (loop (cdr lines) result '() #f)]))
    (let* ([result 
          (if (null? batch)
              result
              (process result batch))]
        [input-port (open-bytevector-input-port (string->utf8 (apply string-append result)))]
