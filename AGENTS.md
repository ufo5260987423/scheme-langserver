...
|----------|-------|--------|
| `scheme-langserver` (general) | `--multi-thread enable` cold initialization hangs (>120 s). Single-thread cold start is slower but completes; multi-thread never returns. Likely related to `threaded-map` + `workspace-mutex` + `make-engine`/`expire` interaction. | **High** — blocks clients that default to multi-thread |
