...
| `scheme-langserver.sls` (multi-thread `exit`) | In multi-threaded mode the LSP `exit` notification was pushed to the request queue and processed on a pool worker; Chez `(exit)` on a fork-thread terminates only that thread, so the main thread stayed blocked on `stdin` and the process never exited (lsp-bridge `lsp_shutdown`/`lsp_restart`/`lsp_close` each burned a 120 s timeout before force-killing; single-thread mode was unaffected). | **Fixed** (2026-09-25) — the main loop now handles `shutdown` and `exit` on the main thread (no queue round-trip): `shutdown` sets the flag and replies immediately; `exit` drains the queue (bounded at 5 s, so a dead consumer cannot hang it), saves the cache, and exits from the main thread. `private:save-workspace-cache-if-any` now takes `workspace-mutex` so the save is serialized with in-flight worker analysis. Regression coverage: multi-thread variants of tests 2/4/5 in `tests/integration/test-shutdown-exit.sps` |
| `scheme-langserver.sls` (MT request pipeline) | Pool worker could die from a runtime-level error (invalid memory reference) escaping `private:try-catch`; all queued requests then starved and the server hung. Observed (2026-09-25): when a condition is raised *and handled inside an engine body* (e.g. `initialize` → `init-workspace` raising permission-denied on a `/tmp` root), Chez's engine stack machinery occasionally corrupts afterwards (~2-5%): `invalid memory reference, some debugging context lost` escapes to the pool and kills the consumer. Pre-existing; not tied to any recent change. | **Mitigated** (2026-09-25) — the consumer loop guards each pop+process with `guard` and logs the escape (`request processor escaped: ...`), so the worker survives and the server stays responsive. Root cause is in Chez engine/exception interaction; a pure-Chez reproducer would be upstream-worthy |
