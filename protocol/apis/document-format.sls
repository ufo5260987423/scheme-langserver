...
(define (document-format workspace params)
  (let* ([text-document (alist->text-document (assq-ref params 'textDocument))]
    ...
    (try
      ...
      (except e 
        [else (vector (text-edit->alist-with-newText (make-text-edit (make-range 0 (string-length text)) text)))]))))
  ...
  (let loop ([body list-of-index-nodes] 
    ...
    (if (null? body)
      (let* ([line-start (private-line-start result)]
        ...
        (substring output-text 0 append-start))
      ...
      [insert-spaces 
        (string-append 
          ...
          (cond 
            ...
            [else " "]))])
    (if (null? children)
      (append output-text insert-spaces (substring origin-text start end))
