...
;;TODO: 用racket的方法，因为close guile没实现
; (define (text-document/did-open! server params)
;     (let* ([text-document (alist->text-document params)]
;             [uril (text-document-uri text-document)]
;             [path (uri->path uri)]
;这里本质上是要将加入的文档进行解析，以实现其他功能包括hover change等等
;             )
        
;         (eq-hashtable-set! (server-open-document server) (string->symbol uri) )))
...
(define (text-document/did-close! params)
...
(define (textDocument/definition server id params)
  (match params
    [(hash-table ['textDocument (DocIdentifier #:uri uri)]
