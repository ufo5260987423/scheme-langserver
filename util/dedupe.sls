...
;; Fast O(n) dedupe using a hash set when the equality predicate is `equal?` or `eq?`.
...
(define (private:dedupe-eq-hashtable e)
    ...
    (case-lambda
        ...
        [(e equal-procedure)
            (cond
                ...
                [(eq? equal-procedure eq?)
                    (private:dedupe-eq-hashtable e)]
                [(null? e)
                    '()]
                [else
                    (cons
...
;; Fast O(n+m) dedupe-deduped using a hash set when the equality predicate is `equal?` or `eq?`.
...
(define (private:dedupe-deduped-eq-hashtable e1 e2)
    (let ([ht (make-eq-hashtable)])
      (for-each (lambda (x) (hashtable-set! ht x #t)) e1)
      (append e1
          (filter (lambda (x) (not (hashtable-contains? ht x)))
    ...
    (case-lambda
        ...
        [(e1 e2 equal-procedure)
            (cond
                ...
                [(eq? equal-procedure eq?)
                    (private:dedupe-deduped-eq-hashtable e1 e2)]
                [else
                    (append e1
